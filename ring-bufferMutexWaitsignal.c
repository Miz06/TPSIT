...
#define N 16   // dimensione del ring

// Struttura del buffer nel ring
typedef struct
{
...
// Flag che indica la fine della lettura
bool end = false;
// Mutex per la sezione critica e per il contatore di blocchi
...
void *leggi(void *par)
{
    ...
    while (!feof(origine))
    {
        ...
        if (n > 0) // Se sono stati letti byte
        {
            ...
            // Blocca il mutex per modificare il contatore dei blocchi
            pthread_mutex_lock(&mutex);
            // Incrementa il contatore dei blocchi nel ring
            n_block++;
            // Sblocca il mutex
...
void *scrivi(void *par)
{
    while (1)
    {
        ...
        if (n_block > 0)
        {
            ...
            // Blocca il mutex per modificare il contatore dei blocchi
            pthread_mutex_lock(&mutex);
            // Decrementa il contatore dei blocchi nel ring
            n_block--;
            // Sblocca il mutex
            pthread_mutex_unlock(&mutex);
